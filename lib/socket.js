...
Socket.prototype.upgrade = function(callback) {
  ...
  Object.keys(extensions).forEach(function(token) {
    var paramsList = extensions[token];
    switch (token) {
    case 'permessage-deflate':
    case 'x-webkit-deflate-frame':
      if (this._extensions.hasOwnProperty(token)) return;

      this._deflateEnabled = true;
      var _params = this._extensions[token] = {};
      paramsList.forEach(function(params) {
        Object.keys(params).forEach(function(key) {
          if (_params.hasOwnProperty(key)) return;

          var value = params[key];
          switch (key) {
          case 'client_max_window_bits':
            if ('string' === typeof value) {
              _params[key] = parseInt(value, 10);
            }
            break;
  ...
  this._deflateParams = this._extensions['permessage-deflate']
      || this._extensions['x-webkit-deflate-frame'] || {};
...
Socket.prototype._parseExtensions = function() {
  ...
  //   Sec-WebSocket-Extensions: permessage-foo; use_y, permessage-foo

  var extensions = {};
  var extensionsStr = this.req.headers['sec-websocket-extensions'] || '';
  extensionsStr.split(',').some(function(extensionStr) {
    var paramsStr = extensionStr.split(';');
    if (!paramsStr.length) {
      this._abortConnectiona();
      return true;
    }

    var token = paramsStr.shift().trim();
    var paramsList = extensions[token] = extensions[token] || [];
    var params = {};
    paramsList.push(params);

    paramsStr.forEach(function(paramStr) {
...
Socket.prototype._formatExtensions = function() {
  return Object.keys(this._extensions).map(function(token) {
    var params = this._extensions[token];
    return [token].concat(Object.keys(params).map(function(key) {
      var value = params[key];
      return true === value ? key : key + '=' + value;
    })).join('; ');
  }, this).join(', ');
};
...
Socket.prototype._inflate = function(data, callback) {
  if (!this._inflator || this._deflateParams['client_no_context_takeover']) {
    var clientMaxWindowBits = this._deflateParams['client_max_window_bits'];
...
Socket.prototype._deflate = function(data, callback) {
  if (!this._deflator || this._deflateParams['server_no_context_takeover']) {
    this._deflator = zlib.createDeflateRaw({
      flush: zlib.Z_SYNC_FLUSH,
      windowBits: this._deflateParams['server_max_window_bits'] || 15,
      memLevel: 8
    });
  }
